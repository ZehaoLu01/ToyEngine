...
void main() {
    // HINT: Change this to output the position in local coordinate system to the fragment shader
    pointPosition = position;

    // HINT: Compute the light direction in VCS
    lightDirection = normalize(viewMatrix * vec4(spherePosition, 1.0) - viewMatrix * modelMatrix * vec4(position, 1.0)).xyz;
